...
2. サブモジュール毎の設計は `documents/design/<submodule>/README.md` に一覧を置く。
3. 個別設計へのリンクは `documents/design/<submodule>/README.md` から直接たどれるようにする。
...
2. `documents/design/README.md` と各 `documents/design/<submodule>/README.md` を整備して 3 参照以内ルールを満たす。
3. `scripts/tools/organize_designs.py` を定期実行する CI（週次）を追加し、dr y-run → レポート → 人間レビューフローを確立する。
4. 古い設計ファイルの完全削除は、レビュー完了後に実施する（現在は削除済みの完全重複のみ実行）。
