...
### 3.1 HLO 解析ログ（追加）
- HLO 解析は **重い処理**になり得るため、原則としてフラグで有効化します。
- 形式は通常ログと同様に **JSONL（1 行 1 JSON）** とします。
- 推奨キー例:
	- `case`: `"hlo"`
	- `tag`: 解析対象の識別子（例: `"minres"`, `"pcg"`）
	- `dialect`: `"stablehlo"` / `"hlo"`
	- `hlo`: HLO 文字列（巨大になり得るため、取り扱いに注意）
...
- 角括弧 `[]` を **必ず**入れます。
- ディレクトリ内に、実行ログ（例: `pytest.txt`）と終了コード（例: `exit_code.txt`）を保存します。

## 6. 禁止事項
- バイナリ形式でのログ出力は禁止します。
- 1 行に複数レコードを詰めないでください。
- 既存ログへの追記は許可しますが、**フォーマットを途中で変更しない**でください。
