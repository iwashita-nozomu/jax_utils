...
2. Mover Agent が `organize_designs.py` を実行してコピーを作成し、`reports/design_organize_report.txt` を出力。
3. Template Agent が検出されたサブモジュールごとに `create_design_template.py` を実行して `template.md` を作る。
4. Reviewer Agent がコピー先を確認、不要なら差し戻し。
5. Coordinator がコミットと PR を作成（ドキュメントだけ main へ反映）。
