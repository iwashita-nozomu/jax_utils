...
- 新規のレビュー文書は、原則として `./reviews/` 以外へ置きません。

## 2. ファイル名

- 新規のレビュー文書ファイル名には、作成したエージェントまたはツールの識別子を必ず入れます。
- 形式は `<TOPIC>__<agent>.md` を原則とします。
- 現在有効なレビューであることを前面に出したい場合は、`CURRENT_<TOPIC>__<agent>.md` を使ってよいです。
- 例:
...
- 既存の歴史的文書は、互換性のため直ちに改名しなくてよいですが、今後追加する文書はこの規則に従います。
...
- 更新時は、古い文章を抱えたまま追記して膨らませるのではなく、必要なら本文を整理して現在有効な状態だけを残します。
