...
from error_handler import ExecutionResult, ErrorCode, Severity
...
class TripletValidator:
    ...
    def report_markdown(self) -> str:
        ...
        lines.append(f"|----------|-----|-----|")
        lines.append(
            f"| ✅ 完全 (Doc+Test) | {complete} | {100*complete/total:.1f}% |"
        )
        lines.append(f"| ⚠️ 部分 (Doc or Test) | {partial} | {100*partial/total:.1f}% |")
        lines.append(f"| ❌ 欠落 (neither) | {missing} | {100*missing/total:.1f}% |")
