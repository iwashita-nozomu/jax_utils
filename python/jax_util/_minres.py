...
def pminres_solve(
    ...
    *,
    ...
    maxiter: int,
    rtol: Scalar = jnp.asarray(EPS, dtype=DEFAULT_DTYPE),
    atol: Scalar = jnp.asarray(ZERO, dtype=DEFAULT_DTYPE),
    ...
    tol_true = jnp.maximum(atol, rtol * bnorm)
...
def _make_sym_indef_system(key: Any, n: int) -> Tuple[Matrix, Vector, Vector]:
    k1, k2 = jax.random.split(key)
    Q, _ = jnp.linalg.qr(jax.random.normal(k1, (n, n)))
    pos = jnp.linspace(1.0, 10.0, n // 2)
    neg = -jnp.linspace(1.0, 10.0, n - n // 2)
    evals = jnp.concatenate([pos, neg])
    A = Q @ (evals[:, None] * Q.T)
...
def _rel_err(x: Vector, x_true: Vector) -> Scalar:
    return jnp.linalg.norm(x - x_true) / (jnp.linalg.norm(x_true) + EPS)


def test_pminres_spd_A_with_jacobi():
    key = jax.random.PRNGKey(0)
    n = 80
    k1, k2 = jax.random.split(key)

    B = jax.random.normal(k1, (n, n))
    A = B.T @ B + jnp.asarray(WEAK_EPS, dtype=DEFAULT_DTYPE) * jnp.eye(n, dtype=DEFAULT_DTYPE)

    x_true = jax.random.normal(k2, (n,))
    b = A @ x_true
    Mv = _dense_Mv_from_A(A)

    d = jnp.abs(jnp.diag(A)) + jnp.asarray(WEAK_EPS, dtype=DEFAULT_DTYPE)

    def Minv(v: Vector) -> Vector:
        return v / d

    st = MINRESState.initialize(x0=jnp.zeros_like(b))
    x_approx, _, info = pminres_solve(
        Mv=Mv,
        rhs=b,
        minres_state=st,
        Minv=LinOp(Minv),
    ...
    print("SPD+Jacobi PMINRES info:", info)
    re = _rel_err(x_approx, x_true)
    print(f"rel error = {float(re):.3e}")
    assert float(info["final_rel_r"]) < 1e-10
    assert float(re) < 1e-6


def test_minres_indef_A_no_precond():
    key = jax.random.PRNGKey(1)
    n = 80
    A, x_true, b = _make_sym_indef_system(key, n)
    Mv = _dense_Mv_from_A(A)

    def Minv(v: Vector) -> Vector:
        return v  # identity SPD
    ...
    print("Indef (no-precond) MINRES info:", info)
    re = _rel_err(x_approx, x_true)
    print(f"rel error = {float(re):.3e}")
    assert float(info["final_rel_r"]) < 1e-8
    assert float(re) < 1e-4
...
def test_rhs_zero():
    ...
    b = jnp.zeros((n,), dtype=DEFAULT_DTYPE)
    ...
    print("rhs=0 info:", info)
    assert float(jnp.linalg.norm(x_approx)) < 1e-12
