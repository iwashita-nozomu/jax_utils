...
def _visible_gpu_ids_from_environment(
    ...
    """
    環境変数からユーザーが指定した可視 GPU ID のリストを解釈して返す。

    戻り値の意味:
    - None: 環境変数未設定 → システム上の全 GPU を対象に検出する（後続で nvidia-smi を参照）
    - (): 空集合または無効指定（"", "-1", "none", "void"）→ GPU を使わない
    - (id1, id2, ...): カンマ区切りで指定された GPU ID のタプル

    フォーマットが不正な場合は ValueError を投げる。
    """
    ...
    for env_name in _GPU_ENV_NAMES:
        ...
        if stripped_value in {"", "-1", "none", "void"}:
            # 明示的に GPU を無効化している
            return ()
        if stripped_value == "all":
            # 全 GPU を対象（後続の検出処理に委ねる）
            return None
    ...
    # どの環境変数も設定されていない場合は None を返す（システム検出に任せる）
    return None
...
def _query_gpu_rows_from_system(
    ...
    try:
        ...
        visible_gpu_ids = _visible_gpu_ids_from_environment(environ)
        if visible_gpu_ids not in {None, ()}:
