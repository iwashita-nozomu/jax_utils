...
class ResourceTotals:
    """
    ユーザがリソース総量を指定するための小さなラッパークラス。

    初期化は `mode` 文字列により動作が切り替わります:
    - "auto" / "from_system": システム検出を行い `FullResourceCapacity.from_system` を使用します。
    - "manual": 引数で与えた `max_workers` 等を利用して手動で `FullResourceCapacity` を構築します。
    - 予約済みのプリセット名 ("tiny", "small", "default", "large"): 内部定義の値を使用します。

    簡易なファクトリとして振る舞い、`capacity` プロパティで `FullResourceCapacity` を返します。
    """

    _PRESETS = {
        "tiny": dict(max_workers=1, host_memory_bytes=256 * 1024),
        "small": dict(max_workers=2, host_memory_bytes=1024 * 1024),
        "default": dict(max_workers=4, host_memory_bytes=4 * 1024 * 1024),
        "large": dict(max_workers=16, host_memory_bytes=32 * 1024 * 1024),
    }
    ...
    def __init__(
        ...
        *,
        ...
        mode = (mode or "").lower()
        if mode in {"auto", "from_system"}:
            # システム検出を使う。
            self._capacity = FullResourceCapacity.from_system(
                max_workers=max_workers,
                host_memory_bytes=host_memory_bytes,
                gpu_devices=gpu_devices,
                environ=environ,
                gpu_query_rows=gpu_query_rows,
                gpu_max_slots=gpu_max_slots,
            )
        elif mode == "manual":
            # 明示的に与えられた値を優先して構築する。
            if max_workers is None:
                raise ValueError("manual mode requires max_workers to be provided")
            self._capacity = FullResourceCapacity(
                max_workers=max_workers,
                host_memory_bytes=host_memory_bytes or 0,
                gpu_devices=tuple(gpu_devices) if gpu_devices is not None else (),
            )
        elif mode in self._PRESETS:
            preset = self._PRESETS[mode]
            preset_max_workers = preset["max_workers"]
            preset_host_memory_bytes = preset["host_memory_bytes"]
            # GPU 検出は環境に委ねる（必要なら query_rows を渡してテスト可能）
            detected_gpus = (
                detect_gpu_devices(environ=environ, query_rows=gpu_query_rows, max_slots=gpu_max_slots)
                if gpu_devices is None
                else tuple(gpu_devices)
            )
            self._capacity = FullResourceCapacity(
                max_workers=max_workers or preset_max_workers,
                host_memory_bytes=host_memory_bytes or preset_host_memory_bytes,
                gpu_devices=tuple(detected_gpus),
            )
        else:
            raise ValueError(f"unknown mode: {mode}")

    @property
    def capacity(self) -> FullResourceCapacity:
        return self._capacity

    def __repr__(self) -> str:  # pragma: no cover - trivial
        return f"ResourceTotals({self._capacity!r})"
