"""Subprocess-based scheduler helpers for experiment scripts.

This module keeps the lighter-weight host/child orchestration utilities used by
older benchmark-style experiments such as `smolyak_scaling`. The newer
`StandardRunner` stack remains the default path for general experiments, while
these helpers provide:

- explicit worker slots
- host-driven subprocess spawning
- JSONL append helpers
- completion records sent back over stdout
"""
...
class WorkerSlot:
    """Execution slot assigned by the host-side subprocess scheduler."""
...
def worker_slot_from_mapping(payload: Mapping[str, object], /) -> WorkerSlot:
    """Deserialize a `WorkerSlot` from a JSON-compatible mapping."""
...
def build_worker_slots(
    ...
    """Build worker slots for either CPU or GPU subprocess execution."""
    if platform == "gpu":
...
def json_compatible(value: object, /) -> object:
    """Convert numpy or JAX-like values into JSON-compatible Python objects."""
...
def run_cases_with_subprocess_scheduler(
    ...
    *,
    ...
    on_case_finished: Callable[
        [Mapping[str, object], WorkerSlot, Mapping[str, object]], None
    ]
    | None = None,
        ...
        for pid, child in running_children.items():
            ...
            if completion is None:
                result = build_parent_failure_result(
                    ...
                    child.worker_slot,
                    "worker_terminated",
                    f"Child process exited with returncode={returncode} without completion record.",
                    (stdout_text + "\n" + stderr_text).strip(),
                )
                if fallback_jsonl_output_path is not None:
                    append_jsonl_record(fallback_jsonl_output_path, result)
