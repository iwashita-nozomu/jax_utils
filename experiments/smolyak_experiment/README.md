...
`experiment_runner` に対してこの topic が実装している要素は次です。

- `task`
- `cases`
- `context_builder` と環境初期化
- `resource_estimate`
- `SkipController` は未使用
...
- `StandardWorker` が `TaskContext["environment_variables"]` を child の先頭で適用してから task を実行します。
- JAX / XLA env の標準設定は `jax_util.xla_env` を使って組み立てます。
