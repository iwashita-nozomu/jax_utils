...
- 現行コードでは、各ケースで
  1. CPU 上で積分器を初期化し、
  2. GPU へ転送し、
  3. 精度確認用の batched integral を回し、
  4. warmup として `num_repeats=100` の repeated integral を 1 回回し、
  5. さらに timed 用に同じ `num_repeats=100` をもう 1 回回す。
...
2. `jax.device_put(...)` で target device へ転送している。
...
2. integrator 初期化と device transfer
3. accuracy evaluation と warmup repeated batch
...
2. warmup repeated batch を 100 回から 1 回ないし数回へ落とす
3. accuracy evaluation と timing run を分離する
4. timeout を child whole-wall ではなく phase-aware に観測する
...
2. child process の使い捨てをやめる、または同形状ケースをまとめる
3. `_difference_rule_storage_numpy` の一時配列重複を減らす
4. 最後の軸以外にも chunking / batching を入れる
