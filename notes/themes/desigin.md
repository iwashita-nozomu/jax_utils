...
- jax.flatten_util.ravel_pytree では $$\Theta \to {\mathbb{R}^n , S}$$ が提供される。
- tree_util.unflatten では $${\mathbb{R}^n , S} \to \Theta$$ である。

```python
leaves, unravel_fn = jax.flatten_util.ravel_pytree(tree)
tree = unravel_fn(leaves)
```
...
[2] Hinton, G. E., Osindero, S., & Teh, Y. W. (2006). "A Fast Learning Algorithm for Deep Belief Nets". *Neural Computation*, 18(7), 1527–1554. [https://doi.org/10.1162/neco.2006.18.7.1527](https://doi.org/10.1162/neco.2006.18.7.1527)
